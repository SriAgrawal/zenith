...
@interface EDKeyboardAvoidingState : NSObject
...
@implementation UIScrollView (EDKeyboardAvoidingAdditions)

- (EDKeyboardAvoidingState*)keyboardAvoidingState {
    EDKeyboardAvoidingState *state = objc_getAssociatedObject(self, &kStateKey);
    if ( !state ) {
        state = [[EDKeyboardAvoidingState alloc] init];
...
- (void)EDKeyboardAvoiding_keyboardWillShow:(NSNotification*)notification {
    ...
    EDKeyboardAvoidingState *state = self.keyboardAvoidingState;
    
    if ( state.keyboardVisible ) {
        return;
    }
    
    UIView *firstResponder = [self EDKeyboardAvoiding_findFirstResponderBeneathView:self];
    ...
    if ( [self isKindOfClass:[EDKeyboardAvoidingScrollView class]] ) {
        ...
        if ( CGSizeEqualToSize(self.contentSize, CGSizeZero) ) {
            ...
            self.contentSize = [self EDKeyboardAvoiding_calculatedContentSizeFromSubviewFrames];
        }
    }
...
-(void)EDKeyboardAvoiding_scrollToActiveTextField {
    ...
    CGPoint idealOffset = CGPointMake(0, [self TPKeyboardAvoiding_idealOffsetForView:[self EDKeyboardAvoiding_findFirstResponderBeneathView:self]
...
- (UIView*)EDKeyboardAvoiding_findFirstResponderBeneathView:(UIView*)view {
    ...
    for ( UIView *childView in view.subviews ) {
        ...
        UIView *result = [self EDKeyboardAvoiding_findFirstResponderBeneathView:childView];
...
- (void)EDKeyboardAvoiding_assignTextDelegateForViewsBeneathView:(UIView*)view {
    for ( UIView *childView in view.subviews ) {
        if ( ([childView isKindOfClass:[UITextField class]] || [childView isKindOfClass:[UITextView class]]) ) {
            ...
            [self EDKeyboardAvoiding_assignTextDelegateForViewsBeneathView:childView];
        }
    }
}
...
@implementation EDKeyboardAvoidingState
@end
